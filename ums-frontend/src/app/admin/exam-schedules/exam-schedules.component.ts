        ...
        <div class="page-content">
          ...
          <div class="d-flex gap-2 mb-3 flex-wrap">
            <div class="search-box" style="max-width:340px;flex:1">
              ...
              <input class="form-control" [(ngModel)]="searchQ" (ngModelChange)="onFilterChange()"
                     placeholder="ค้นหารหัสวิชา, ชื่อวิชา, อาจารย์..." />
            </div>
            <select class="form-select" style="max-width:180px" [(ngModel)]="filterDept" (ngModelChange)="onFilterChange()">
              <option value="">ทุกแผนก</option>
              <option *ngFor="let d of depts()" [value]="d.dept_id">{{ d.name }}</option>
            </select>
            <select class="form-select" style="max-width:150px" [(ngModel)]="filterType" (ngModelChange)="onFilterChange()">
          ...
          @if (activeTab() === 'has-exam') {
            ...
            <div class="d-flex justify-content-between align-items-center mt-3" *ngIf="!loading() && examTotal() > 0">
              <span class="text-muted small">แสดง {{ exams().length }} จาก {{ examTotal() }} รายการ</span>
              <div class="d-flex gap-2">
                <button class="btn btn-sm btn-outline-secondary" [disabled]="examPage() === 1" (click)="goExamPage(examPage()-1)">‹</button>
                <button *ngFor="let p of examPages()" class="btn btn-sm" [class]="p === examPage() ? 'btn-primary' : 'btn-outline-secondary'" (click)="goExamPage(p)">{{ p }}</button>
                <button class="btn btn-sm btn-outline-secondary" [disabled]="examPage() === examTotalPages()" (click)="goExamPage(examPage()+1)">›</button>
              </div>
            </div>
          }
          ...
          @if (activeTab() === 'no-exam') {
            <div class="card">
              ...
              <div class="table-responsive" *ngIf="!noExamLoading()" style="margin-bottom:0">
            ...
            <div class="d-flex justify-content-between align-items-center mt-3" *ngIf="!noExamLoading() && noExamTotal() > 0">
              <span class="text-muted small">แสดง {{ noExamList().length }} จาก {{ noExamTotal() }} รายการ</span>
              <div class="d-flex gap-2">
                <button class="btn btn-sm btn-outline-secondary" [disabled]="noExamPage() === 1" (click)="goNoExamPage(noExamPage()-1)">‹</button>
                <button *ngFor="let p of noExamPages()" class="btn btn-sm" [class]="p === noExamPage() ? 'btn-primary' : 'btn-outline-secondary'" (click)="goNoExamPage(p)">{{ p }}</button>
                <button class="btn btn-sm btn-outline-secondary" [disabled]="noExamPage() === noExamTotalPages()" (click)="goNoExamPage(noExamPage()+1)">›</button>
              </div>
            </div>
          }
        </div>
...
export class AdminExamSchedulesComponent implements OnInit {
  ...
  // Pagination — has-exam tab
  examPage       = signal(1);
  examTotal      = signal(0);
  examTotalPages = computed(() => Math.max(1, Math.ceil(this.examTotal() / 20)));
  examPages      = computed(() => {
    const t = this.examTotalPages(), c = this.examPage();
  ...
  // Pagination — no-exam tab
  noExamPage       = signal(1);
  noExamTotal      = signal(0);
  noExamTotalPages = computed(() => Math.max(1, Math.ceil(this.noExamTotal() / 20)));
  noExamPages      = computed(() => {
    const t = this.noExamTotalPages(), c = this.noExamPage();
    const start = Math.max(1, Math.min(c - 2, t - 4));
    return Array.from({ length: Math.min(5, t) }, (_, i) => start + i);
  });
  ...
  loadNoExamList() {
    ...
    const params: any = { page: this.noExamPage(), limit: 20 };
    if (this.searchQ)    params.search  = this.searchQ;
    if (this.filterDept) params.dept_id = this.filterDept;
    this.api.getNoExamList(params).subscribe({
      next: (r: any) => {
        this.noExamList.set(r.data?.data ?? r.data ?? []);
        this.noExamTotal.set(r.data?.total ?? 0);
        this.noExamLoading.set(false);
      },
  ...
  load() {
    ...
    const params: any = { page: this.examPage(), limit: 20 };
    if (this.searchQ)    params.search    = this.searchQ;
    if (this.filterDept) params.dept_id   = this.filterDept;
    ...
    this.api.getExamSchedules(params).subscribe({
      next: (res: any) => {
        this.exams.set(res.data?.data ?? res.data ?? []);
        this.examTotal.set(res.data?.total ?? 0);
  ...
  goExamPage(p: number) { this.examPage.set(p); this.load(); }
  goNoExamPage(p: number) { this.noExamPage.set(p); this.loadNoExamList(); }

  onFilterChange() {
    this.examPage.set(1);
    this.noExamPage.set(1);
    this.load();
    if (this.activeTab() === 'no-exam') this.loadNoExamList();
  }
